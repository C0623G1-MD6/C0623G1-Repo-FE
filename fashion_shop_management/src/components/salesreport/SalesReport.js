...
function SalesReport() {
  ...
  const getSearchSalesReport = async () => {
    setSalesReportProduct(
      await SalesReportService.getAllSreach(startDate, endDate)
    );
  };
  ...
  const labels = salesReportProduct.map((salesReport) =>
    formatDateString(new Date(salesReport.date))
  );
  const dataRevenue = salesReportProduct.map(
    (salesReport) => salesReport.revenue
  );
